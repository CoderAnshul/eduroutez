...
const BlogReviewForm = ({ blog }) => {
  ...
  const { data: reviewsData, isLoading, refetch } = useQuery(
    ['blogReviews', blog._id],
    async () => {
      const response = await axiosInstance.get(`/api/v1/blog-reviews/${blog._id}`);
      return response.data;
    },
    {
      enabled: Boolean(blog._id),
      onSuccess: (data) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post('http://localhost:4001/api/v1/submit-review', {
        id: blog._id,
        rating,
        comment,
      }, {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h4 className="text-xl font-semibold">Reader Reviews</h4>
            ...
            {reviews.map((review) => (
              <div key={review._id} className="border-b pb-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h6 className="font-medium">{review.userName || 'Anonymous'}</h6>

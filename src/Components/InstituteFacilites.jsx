...
const InstituteFacilities = ({ instituteData }) => {
  ...
  if (
    !instituteData?.data?.facilities ||
    instituteData.data.facilities.length === 0
  ) {
    return null; // Hide the section if there are no facilities
  }

  const facilitiesList = instituteData.data.facilities.map(
    (facility, index) => ({
      name: facility,
      color: [
        "#FEF2F2",
